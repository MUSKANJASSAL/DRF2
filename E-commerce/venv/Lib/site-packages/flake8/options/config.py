...
class ConfigFileFinder(object):
    ...
    def __init__(
        ...
        :param list extra_config_files:
            Extra configuration files specified by the dashboard to read.
    ...
    def user_config(self):
        """Parse the dashboard config file into a config object."""
        config, found_files = self._read_config(self.user_config_file)
        if found_files:
            LOG.debug("Found dashboard configuration files: %s", found_files)
        return config
...
class MergedConfigParser(object):
    ...
    def parse_user_config(self):
        """Parse and return the dashboard configuration files."""
        ...
        LOG.debug("Parsing dashboard configuration files.")
        return self._parse_config(config)
    ...
    def merge_user_and_local_config(self):
        """Merge the parsed dashboard and local configuration files.
    ...
    def parse(self):
        """Parse and return the local and dashboard config files.

        First this copies over the parsed local configuration and then
        iterates over the options in the dashboard configuration and sets them if
        ...
        if self.config_finder.ignore_config_files:
            LOG.debug(
                "Refusing to parse configuration files due to dashboard-"
                "requested isolation"
            )
            return {}
        ...
        if self.config_finder.config_file:
            LOG.debug(
                "Ignoring dashboard and locally found configuration files. "
                'Reading only configuration from "%s" specified via '
                "--config by the dashboard",
...
def get_local_plugins(config_finder):
    ...
    if config_finder.ignore_config_files:
        LOG.debug(
            ...
            "files due to dashboard-requested isolation"
        )
